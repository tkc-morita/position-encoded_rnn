...
from train_sort import Learner as _Learner
...
class Learner(_Learner):
	def train_per_iteration(self, sequence, records, iteration):
		...
		ar_input = torch.cat([torch.full((sequence.size(0),1), vocab_size, device=self.device),
								sorted[:,:-1]], dim=1)

		logits = self.rnn(sequence, ar_input=ar_input)
	...
	def test(self, sequence):
		...
		sorted = sequence.sort(dim=1)[0]
		logits = self.rnn(sequence) # NOTE: No ground-truth feedback.
		logits = logits[:,sequence.size(1):,:] # Strip-off the encoding phase.
		...
		# Ground-truth feedback (for comparison)
		vocab_size = self.checkpoint['modules']['rnn']['init_args']['vocab_size']
		ar_input = torch.cat([torch.full((sequence.size(0),1), vocab_size, device=self.device),
								sorted[:,:-1]], dim=1)
		logits = self.rnn(sequence, ar_input=ar_input)
		...
		token_accuracy = is_correct.float().mean().item()
		seq_accuracy = is_correct.all(dim=-1).float().mean().item()
		self.logger.info('Test accuracy (token; ground-truth feedback): {}'.format(token_accuracy))
		self.logger.info('Test accuracy (sequence-wise full-match; ground-truth feedback): {}'.format(seq_accuracy))
