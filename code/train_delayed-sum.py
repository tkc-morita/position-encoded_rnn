...
class Learner(_Learner):
	def train_per_iteration(self, whole, records, iteration):
		self.optimizer.zero_grad()
		whole = whole.to(self.device)
		prefix,suffix = whole.chunk(2, dim=-1)

		vocab_size = self.checkpoint['modules']['rnn']['init_args']['vocab_size']
		target = (prefix.flip(dims=(1,))+suffix) % vocab_size

		logits = self.rnn(whole)
		logits = logits[:,-target.size(1):,:] # Strip-off the prefix output
	...
	def test(self, whole):
		...
		perplexity = F.cross_entropy(logits.reshape(-1,vocab_size), target.view(-1)).exp().item()
		...
		self.logger.info('Test perplexity: {}'.format(perplexity))
...
if __name__=='__main__':
	...
	logger.info('Learns delayed summation.')
	logger.info('Vocabulary size: {}'.format(args.vocab_size))
	logger.info('(Half) sequence length: {}'.format(args.seq_length))
	...
	dataset = RandomSequence(args.vocab_size, args.seq_length*2, args.num_held_out, dummy_datasize=max(512,args.batch_size))
